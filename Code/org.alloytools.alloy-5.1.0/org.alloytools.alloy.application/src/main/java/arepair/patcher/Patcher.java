package arepair.patcher;

import static arepair.clean.ModelSimplifier.simplify;
import static arepair.patcher.etc.Constants.BOUND_TYPE;
import static arepair.patcher.etc.Constants.CLI_USAGE_DESCRIPTION_WIDTH;
import static arepair.patcher.etc.Constants.DEFAULT_NUM_TRY_NUM_PER_DEPTH;
import static arepair.patcher.etc.Constants.DEFAULT_NUM_TRY_PER_HOLE;
import static arepair.patcher.etc.Constants.DEFAULT_PARTITION_NUM;
import static arepair.patcher.etc.Constants.ENABLE_CACHE;
import static arepair.patcher.etc.Constants.FIX_FILE_PATH;
...
import static arepair.patcher.etc.Constants.MAX_TRY_NUM_PER_DEPTH;
import static arepair.patcher.etc.Constants.MAX_TRY_PER_HOLE;
import static arepair.patcher.etc.Constants.MINIMUM_COST;
import static arepair.patcher.etc.Constants.MODEL_PATH;
import static arepair.patcher.etc.Constants.PARTITION_NUM;
import static arepair.patcher.etc.Constants.SCOPE;
import static arepair.patcher.etc.Constants.SEARCH_STRATEGY;
import static arepair.patcher.etc.Constants.SUSPICIOUSNESS_THRESHOLD;
import static arepair.patcher.etc.Constants.TEST_PATH;
...
public class Patcher {
    ...
    public static void patch(PatcherOpt opt) {
        ...
        while (canFix) {
            ...
            List<TestResult> testResults = TestRunner.runTests(modelToFix, opt.getTestSuitePath(), opt.getPSV(), opt.getOptions());
            ...
            if (mostSuspiciousImpact.getFailToPass() > 0 && mostSuspiciousImpact.getPassToFail() == 0 || mostSuspiciousImpact.getScore() > SUSPICIOUSNESS_THRESHOLD) {
                ...
                logger.debug("Score: " + mostSuspiciousImpact.getScore() + ", FailToPass: " + mostSuspiciousImpact.getFailToPass() + ", PassToFail: " + mostSuspiciousImpact.getPassToFail());
    ...
    private static void printAlloyPatcherUsage() {
        logger.info("Patcher requires: model path, test path, scope, lowest cost, search strategy" + " and whether to enable sub-formula caching.  If the search strategy is base-choice," + " then --" + MAX_TRY_PER_HOLE + " must be set.  If the search strategy is all-combinations," + " then --" + PARTITION_NUM + " and --" + MAX_TRY_NUM_PER_DEPTH + " must be set.");
    }

    public static void printARepairUsage(HelpFormatter formatter, Options options) {
        formatter.setOptionComparator(null);
        formatter.printHelp(CLI_USAGE_DESCRIPTION_WIDTH, Patcher.class.getSimpleName(), "Repair faulty Alloy models.", options, null, true);
    }
    ...
    private static PatcherOpt parseCommandLineArgs(String[] args) {
        ...
        options.addRequiredOption("c", MINIMUM_COST, true, "Minimum cost/size of the generate expressions.");
        ...
        try {
            ...
            String modelPath = Paths.get(commandLine.getOptionValue(MODEL_PATH)).toAbsolutePath().toString();
            if (!FileUtil.fileExists(modelPath)) {
                logger.error("Cannot find model at " + modelPath);
                printARepairUsage(formatter, options);
                return null;
            }
            String testPath = Paths.get(commandLine.getOptionValue(TEST_PATH)).toAbsolutePath().toString();
            ...
            if (searchStrategy == null) {
                logger.error("Search strategy must be in " + Arrays.asList(SearchStrategy.values()) + ", but found " + searchStrategyOption);
                printAlloyPatcherUsage();
                return null;
            }
            ...
            return new PatcherOpt(modelPath, testPath, scope, searchStrategy, commandLine.hasOption(ENABLE_CACHE), minimumCost, maxTryPerHole, partitionNum, maxTryPerDepth);
