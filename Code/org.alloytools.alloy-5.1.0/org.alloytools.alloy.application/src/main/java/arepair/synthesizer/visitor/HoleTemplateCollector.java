...
import arepair.synthesizer.hole.BinaryFormulaCmpHole;
import arepair.synthesizer.hole.BinaryFormulaLogicHole;
import arepair.synthesizer.hole.ExprHole;
import arepair.synthesizer.hole.Hole;
import arepair.synthesizer.hole.ListFormulaHole;
import arepair.synthesizer.hole.QuantifierHole;
import arepair.synthesizer.hole.SigHole;
import arepair.synthesizer.hole.UnaryExprCardHole;
import arepair.synthesizer.hole.UnaryFormulaBoolHole;
import arepair.synthesizer.hole.UnaryFormulaCardHole;
...
/**
 * This class returns the AST nodes that represent holes. Note that if an AST
 * node represents a hole, then its subnodes will not be printed out, but
 * instead the candidate value of the hole will be printed out.
 */
public class HoleTemplateCollector implements GenericVisitor<Map<Node,Hole>,Object> {
    ...
    public Map<Node,Hole> visit(ListExpr n, Object arg) {
        ...
        // Return nodes recursively.
        Map<Node,Hole> map = new LinkedHashMap<>();
        n.getArguments().forEach(exprOrFormula -> exprOrFormula.accept(this, arg).forEach(map::putIfAbsent));
        return map;
    }
