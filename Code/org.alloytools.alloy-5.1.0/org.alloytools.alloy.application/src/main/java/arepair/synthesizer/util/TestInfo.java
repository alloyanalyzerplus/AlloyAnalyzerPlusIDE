...
public class TestInfo {
    ...
    /**
     * Maps each fact name and the test predicate name to their result. To make the
     * test pass, all facts and the test predicate must be satisfiable.
     */
    private Map<String,Boolean>             formulaResults;

    private final boolean                   expectSat;
    /**
     * If all of the facts and the test body is satisfiable, then this field is
     * true.
     */
    ...
    /**
     * This field caches Alloy expressions/formulas to their values in a bottom-up
     * fashion. The idea is to only evaluate small expressions/formulas (from bottom
     * of AST) if possible and if the values are encountered before, then we do not
     * need to evaluate the big expression (from top of AST).
     */
    private EvaluationCache                 evaluationCache;

    /**
     * This field maps the valuation of the Alloy paragraph to synthesize to the
     * result of facts and test corresponding to this class. Given the valuation of
     * the paragraph, if it is encountered before, then we do not need to evaluate
     * the facts and test any more.
     */
    ...
    /**
     * Evaluate the formulas and set {@code actualSat}. This method throws exception
     * if any of the evaluation results in an error. This method should never throw
     * an exception.
     */
