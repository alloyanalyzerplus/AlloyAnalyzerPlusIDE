...
public class JRule13 extends BinaryRule {
    ...
    public boolean isPruned() {
        if (opIsOr(leftRel.getOp(), ARROW)) {
            ...
            if (sameRelations(leftSubRel, rightRel) && rightRel.getArity() == 1 && isSuperType(rightRel, inheritanceMap) && !getChild(leftRel, 0).hasIden() && getChild(leftRel, 0).getTypes().contains(rightRel.getTypes().get(0))) {
                return true;
            }
        }
        if (opIsOr(rightRel.getOp(), ARROW)) {
            Expression rightSubRel = getChild(rightRel, 0);
            if (sameRelations(leftRel, rightSubRel) && leftRel.getArity() == 1 && isSuperType(leftRel, inheritanceMap) && !getChild(rightRel, 1).hasIden() && getChild(rightRel, 1).getTypes().contains(leftRel.getTypes().get(0))) {
                return true;
            }
        }
        return false;
